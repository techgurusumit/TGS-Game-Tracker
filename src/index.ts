...
db.exec(`
CREATE TABLE IF NOT EXISTS tournaments (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE, created_at TEXT DEFAULT CURRENT_TIMESTAMP);
CREATE TABLE IF NOT EXISTS players (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL UNIQUE, discord_id TEXT, created_at TEXT DEFAULT CURRENT_TIMESTAMP);
...
function getTournament(name:string){ return db.prepare('SELECT * FROM tournaments WHERE name = ? COLLATE NOCASE').get(name) as any; }
...
async function handle(i: ChatInputCommandInteraction) {
  try {
    ...
    if (i.commandName === 'tournament') {
      ...
      await i.reply(`✅ Tournament **${name}** created.`); return;
    }
    ...
    if (i.commandName === 'race') {
      const tournamentName=i.options.getString('tournament',true).trim(); const number=i.options.getInteger('number',true); const track=i.options.getString('track',true).trim(); const raw=i.options.getString('results',true);
      const t=getTournament(tournamentName); if(!t) { await i.reply('❌ Tournament not found. Create it first with `/tournament`.'); return; }
      const entries=raw.split(',').map(x=>x.trim()).filter(Boolean).map(x=>{const [name,pts]=x.split('=').map(s=>s.trim()); return {name,points:Number(pts)};});
      if(!entries.length || entries.some(x=>!x.name || !Number.isFinite(x.points))) { await i.reply('❌ Invalid results. Use `Player1=25,Player2=18`.'); return; }
      const tx=db.transaction(()=>{
        const race=db.prepare('INSERT INTO races(tournament_id,race_no,track) VALUES(?,?,?)').run(t.id,number,track);
        for(const e of entries){ let p=getPlayer(e.name); if(!p){ const r=db.prepare('INSERT INTO players(name) VALUES(?)').run(e.name); p={id:r.lastInsertRowid,name:e.name}; }
          db.prepare('INSERT INTO results(race_id,player_id,points,position) VALUES(?,?,?,?)').run(race.lastInsertRowid,p.id,e.points, null); }
      }); tx();
      await i.reply(`🏁 Race **${number}** recorded for **${tournamentName}** on **${track}** with **${entries.length} players**.`); return;
    }
    if (i.commandName === 'stats') {
      const name=i.options.getString('player',true); const p=getPlayer(name); if(!p){await i.reply('❌ Player not found.');return;}
      const s=db.prepare('SELECT COALESCE(SUM(points),0) total, COUNT(*) races FROM results WHERE player_id=?').get(p.id) as any;
      const best=db.prepare('SELECT MAX(points) best FROM results WHERE player_id=?').get(p.id) as any;
      await i.reply({embeds:[new EmbedBuilder().setTitle(`🏎️ ${p.name}`).addFields({name:'Total Points',value:String(s.total),inline:true},{name:'Races',value:String(s.races),inline:true},{name:'Best Race Points',value:String(best.best ?? 0),inline:true})]}); return;
    }
    if (i.commandName === 'tournament-stats') {
      const name=i.options.getString('tournament',true); const t=getTournament(name); if(!t){await i.reply('❌ Tournament not found.');return;}
      const rows=db.prepare(`SELECT p.name, SUM(r.points) total, COUNT(r.id) races FROM results r JOIN players p ON p.id=r.player_id JOIN races ra ON ra.id=r.race_id WHERE ra.tournament_id=? GROUP BY p.id ORDER BY total DESC`).all(t.id) as any[];
      const text=rows.length?rows.map((r,n)=>`${n+1}. **${r.name}** — ${r.total} pts (${r.races} races)`).join('\n'):'No race results yet.';
      await i.reply({embeds:[new EmbedBuilder().setTitle(`🏆 ${t.name} Standings`).setDescription(text)]}); return;
    }
    if (i.commandName === 'player-history') {
      const name=i.options.getString('player',true); const p=getPlayer(name); if(!p){await i.reply('❌ Player not found.');return;}
      const rows=db.prepare(`SELECT t.name tournament, ra.race_no, ra.track, r.points FROM results r JOIN races ra ON ra.id=r.race_id JOIN tournaments t ON t.id=ra.tournament_id WHERE r.player_id=? ORDER BY ra.created_at DESC, ra.race_no DESC LIMIT 20`).all(p.id) as any[];
      const text=rows.length?rows.map(r=>`**${r.tournament}** • Race ${r.race_no} • ${r.track} — **${r.points} pts**`).join('\n'):'No races recorded.';
      await i.reply({embeds:[new EmbedBuilder().setTitle(`📋 ${p.name} History`).setDescription(text)]}); return;
    }
  } catch(e:any) { console.error(e); if(i.replied||i.deferred) await i.followUp('❌ Something went wrong. Check the server logs.'); else await i.reply('❌ Something went wrong. Check the server logs.'); }
}

client.once('ready', async c => { console.log(`Logged in as ${c.user.tag}`); });
